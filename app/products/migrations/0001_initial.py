# Generated by Django 4.2.7 on 2023-11-14 20:51

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import django_jalali.db.models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('Category', '0001_initial'),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
        ('images', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='Option',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_at', django_jalali.db.models.jDateTimeField(auto_now_add=True, verbose_name='تاریخ ایجاد')),
                ('title', models.CharField(db_index=True, max_length=50, verbose_name='عنوان')),
                ('create_by', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='توسط کاربر ایجاد شده')),
            ],
            options={
                'verbose_name': 'option',
                'verbose_name_plural': 'options',
                'db_table': 'option',
            },
        ),
        migrations.CreateModel(
            name='OptionValueModel',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(db_index=True, max_length=50, verbose_name='عنوان')),
                ('body_option_value', models.CharField(max_length=100, verbose_name='متن عنوان')),
            ],
            options={
                'verbose_name': 'option value',
                'verbose_name_plural': 'option values',
                'db_table': 'option_value',
            },
        ),
        migrations.CreateModel(
            name='SallerModel',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_at', django_jalali.db.models.jDateTimeField(auto_now_add=True, verbose_name='تاریخ ایجاد')),
                ('update_at', django_jalali.db.models.jDateTimeField(auto_now=True, null=True, verbose_name='تاریخ بروزرسانی')),
                ('product_name', models.CharField(db_index=True, max_length=150, verbose_name='نام کالا')),
                ('price', models.PositiveIntegerField(db_index=True, verbose_name='قیمت')),
                ('warrenty_choose', models.CharField(choices=[('no warrenty', 'no warrenty'), ('company warrenty', 'company warrenty')], default='no warrenty', max_length=16, verbose_name='نوع گارانتی')),
                ('is_stock', models.BooleanField(default=True, verbose_name='موجود هست')),
                ('number_product', models.PositiveSmallIntegerField(verbose_name='تعداد محصول')),
                ('is_delivery', models.BooleanField(default=True, verbose_name='ارسال از طریق پست')),
                ('performance_product', models.CharField(choices=[('very bad', 'very bad'), ('bad', 'bad'), ('normall', 'normall'), ('best', 'best'), ('very best', 'very best')], default='very best', max_length=9, verbose_name='عملکرد کالا')),
                ('create_by', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='توسط کاربر ایجاد شده')),
                ('image', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, related_name='images', to='images.imagesmodel')),
                ('option', models.ManyToManyField(related_name='saller_options', to='products.option')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, related_name='users', to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'saller',
                'verbose_name_plural': 'sellers',
                'db_table': 'saller',
            },
        ),
        migrations.CreateModel(
            name='ProductModel',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_at', django_jalali.db.models.jDateTimeField(auto_now_add=True, verbose_name='تاریخ ایجاد')),
                ('update_at', django_jalali.db.models.jDateTimeField(auto_now=True, null=True, verbose_name='تاریخ بروزرسانی')),
                ('description_product', models.TextField(blank=True, null=True)),
                ('barcode', models.PositiveBigIntegerField(verbose_name='بارکد کالا')),
                ('product_auth_code', models.CharField(max_length=128, unique=True, verbose_name='کد شناسایی هر کالا')),
                ('category', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, related_name='categories', to='Category.category')),
                ('create_by', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL, verbose_name='توسط کاربر ایجاد شده')),
                ('image', models.ManyToManyField(related_name='product_images', to='images.imagesmodel')),
                ('option', models.ManyToManyField(related_name='options', to='products.option')),
                ('saller', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, related_name='sallers', to='products.sallermodel')),
                ('title_product', models.OneToOneField(on_delete=django.db.models.deletion.PROTECT, related_name='title_products', to='products.sallermodel')),
            ],
            options={
                'verbose_name': 'product',
                'verbose_name_plural': 'products',
                'db_table': 'product',
            },
        ),
    ]
